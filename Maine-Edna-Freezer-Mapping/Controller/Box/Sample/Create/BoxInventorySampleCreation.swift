//
//  BoxInventorySampleCreation.swift
...
class BoxInventorySampleCreation: ObservableObject {
   ...
   // @AppStorage(AppStorageNames.stored_freezers.rawValue) var stored_freezers : [FreezerProfileModel] = [FreezerProfileModel]()
    ...
    func CreateInventorySample(_sampleDetail : InventorySampleModel,completion: @escaping (ServerMessageModel) -> Void) -> Void
    {
  
        print("Token: \(self.edna_freezer_token)")
        ///
        let request : String = ServerConnectionUrls.productionUrl.rawValue + "api/freezer_inventory/inventory/"
        ...
        AF.request(request ,
            ...
            if response.response?.statusCode == 400{
                ...
                if let errorMsg = response.data{
                    let jsonMsg = String(data: errorMsg,encoding: String.Encoding.utf8)
                    let errorDetail = ServerMessageModel()
                    errorDetail.serverMessage = "Sample Successfully Saved"//String(describing: jsonMsg!)
